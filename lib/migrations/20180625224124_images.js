...
exports.up = function(knex, Promise) {
    ...
    .createTable( 'layers', function( layersTable ) {
        ...
        // Parent, trace is created and from that SVG multiple layers are saved
        layersTable.integer( 'trace_id' ).notNullable();

        // Parent->Parent Relationship: Layer (belongsTo) => Trace (belongsTo) => Image
        layersTable.integer( 'image_id' ).notNullable();
        
        layersTable.specificType('path', 'blob' ).notNullable();

        //layersTable.string( 'fill' ).nullable().default(null);
        //layersTable.stroke( 'stroke' ).nullable().default(null);
